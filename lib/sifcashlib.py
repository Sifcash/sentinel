...
def is_valid_sifcash_address(address, network='mainnet'):
    ...
    sifcash_version = 140 if network == 'testnet' else 63
    ...
    try:
        ...
        # rescue from exception, not a valid Sifcash address
        return False

    if (address_version != sifcash_version):
        return False
...
# shims 'til we can fix the sifcashd side
def SHIM_serialise_for_sifcashd(sentinel_hex):
    ...
    # superblock => "trigger" in sifcashd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # sifcashd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    sifcashd_hex = serialise(obj)
    return sifcashd_hex


# shims 'til we can fix the sifcashd side
def SHIM_deserialise_from_sifcashd(sifcashd_hex):
    from models import TALCASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(sifcashd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('sifcash.conf')
