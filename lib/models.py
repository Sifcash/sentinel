...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, sifcashd):
        golist = sifcashd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_sifcashd(sifcashd, item)
    ...
    @classmethod
    def import_gobject_from_sifcashd(self, sifcashd, rec):
        import decimal
        import sifcashlib
        import inflection
        ...
        # shim/sifcashd conversion
        object_hex = sifcashlib.SHIM_deserialise_from_sifcashd(object_hex)
        objects = sifcashlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from sifcashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from sifcashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from sifcashd - Sifcashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(sifcashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from sifcashd! %s" % e)
            govobj.vote_delete(sifcashd)
            return (govobj, None)
    ...
    def vote_delete(self, sifcashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(sifcashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, sifcashd, signal, outcome):
        import sifcashlib
        ...
        output = sifcashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = sifcashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(sifcashd, signal)

    def sync_network_vote(self, sifcashd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = sifcashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 sifcash addr, non-multisig
            if not sifcashlib.is_valid_sifcash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Sifcash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (sifcashlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SifcashDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import sifcashlib
        obj_data = sifcashlib.SHIM_serialise_for_sifcashd(self.serialise())
    ...
    def prepare(self, sifcashd):
        try:
            object_hash = sifcashd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not sifcashlib.is_valid_sifcash_address(addr, config.network):
    ...
    def hash(self):
        import sifcashlib
        return sifcashlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, sifcashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - sifcashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, sifcashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - sifcashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, sifcashd):
        now = int(time.time())
        return (self.created_at < (now - sifcashd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, sifcashd):
        if self.is_expired(sifcashd):
            return False

        return True

    def is_deletable(self, sifcashd):
        if self.is_expired(sifcashd):
            return True
