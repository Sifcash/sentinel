...
sentinel_cfg = SifcashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_sifcashd_proto_version_with_sentinel_ping = 70207


def get_sifcash_conf():
    home = os.environ.get('HOME')

    sifcash_conf = os.path.join(home, ".sifcashcore/sifcash.conf")
    if sys.platform == 'darwin':
        sifcash_conf = os.path.join(home, "Library/Application Support/SifcashCore/sifcash.conf")

    sifcash_conf = sentinel_cfg.get('sifcash_conf', sifcash_conf)
