...
def has_sifcash_conf():
    import config
    import io

    valid_sifcash_conf = False

    # ensure sifcash_conf exists & readable
    #
    # if not, print a message stating that Sifcash Core must be installed and
    # configured, including JSONRPC access in sifcash.conf
    try:
        f = io.open(config.sifcash_conf)
        valid_sifcash_conf = True
    except IOError as e:
        print(e)

    return valid_sifcash_conf
...
def main():
    ...
    if not has_sifcash_conf():
        print("SifcashCore must be installed and configured, including JSONRPC access in sifcash.conf")
        sys.exit(1)
