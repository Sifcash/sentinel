...
class SifcashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some sifcashd methods
    ...
    @classmethod
    def from_sifcash_conf(self, sifcash_dot_conf):
        from sifcash_config import SifcashConfig
        config_text = SifcashConfig.slurp_config_file(sifcash_dot_conf)
        creds = SifcashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = sifcashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = sifcashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = sifcashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_sifcashd_proto_version_with_sentinel_ping)
